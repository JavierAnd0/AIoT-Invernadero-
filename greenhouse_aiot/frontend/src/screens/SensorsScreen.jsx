  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>Sensors</h1>
          <div style={{ color: 'var(--text-secondary)', fontSize: 12, marginTop: 2 }}>Historical readings</div>
        </div>
        <Select
          label=""
          value={deviceId || ''}
      ...
      {!rLoading && deviceList.length > 0 && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <Card>
              <div style={{ fontWeight: 600, fontSize: 12, color: 'var(--text-primary)', marginBottom: 12 }}>Temperature (°C)</div>
              <LineChart labels={labels} datasets={[{ label: 'Temp', data: sr.map(r => r.temperature), color: '#f59e0b' }]} />
            </Card>
            <Card>
              <div style={{ fontWeight: 600, fontSize: 12, color: 'var(--text-primary)', marginBottom: 12 }}>Humidity (%)</div>
              <LineChart labels={labels} datasets={[{ label: 'Humidity', data: sr.map(r => r.humidity), color: '#3b82f6' }]} />
            </Card>
            <Card>
              <div style={{ fontWeight: 600, fontSize: 12, color: 'var(--text-primary)', marginBottom: 12 }}>Light (lux)</div>
              <LineChart labels={labels} datasets={[{ label: 'Light', data: sr.map(r => r.light_lux), color: '#eab308' }]} />
            </Card>
            <Card>
              <div style={{ fontWeight: 600, fontSize: 12, color: 'var(--text-primary)', marginBottom: 12 }}>CO₂ (ppm)</div>
